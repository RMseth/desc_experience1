...
\begin{document}
	...
	\begin{enumerate}
		...
		\textbf{Solution: }\\
		Because graph $Q_n$ is bipartite, there is an edge from every x to an edge in y and there is an edge from every y to an edge in x.  Because, there are n edges in $Q_n$ (we're going without a proof) any graph $Q_n$ is strongly connected.  It is Hamiltonian because every vertex has an equal amount of edges.\\
